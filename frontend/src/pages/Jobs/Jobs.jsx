...
const Jobs = () => {
  ...
  const searchHandler = () => {
    const searchArr = baseJobs.filter((e) =>
      e.title.toLowerCase().includes(search.toLowerCase().trim()) &&
      e.employmentType.toLowerCase().includes(employmentType.toLowerCase().trim())
    );

    setJobs(search !== '' ? searchArr : baseJobs);
    setEmploymentType(search!==""?employmentType:"");
  };
  ...
  return (
    <>
      <MetaData title="Jobs" />
      <div className="bg-gray-950 min-h-screen pt-14 sm:px-20 px-3 text-white flex flex-col items-center">
      {loading ? <Loader /> :
      <>
      ...
      <div className={`flex flex-col sm:flex-row justify-center items-center w-full gap-4 py-4 ${isMobile ? 'px-3' : ''}`}>

        {/* Left section for category filter */}
        <div className={`flex justify-start flex-col p-4 rounded w-full sm:w-1/4 ${isMobile ? 'w-full' : 'w-1/4'}`}>
        <p className="text-xl flex justify-center underline underline-offset-4">Categories</p>
        <ul className="flex pt-3 justify-start md:justify-center items-center  mb-3 flex-col gap-3">
        {data.map((e, i) => (
        <li
          key={i}
        ...
        <button onClick={() => leftFilter(jobs)} className="blueCol px-1 py-1 text-sm sm:text-base">Apply Filter</button>
        <button onClick={removeLeftFilter} className="blueCol px-1 py-2 text-xs sm:text-sm">Remove Filter</button>
        </div>
        </div>

        {/* Center section for job search input and job cards */}
        <div className={`flex-1 p-4 ${isMobile ? 'w-full' : ''}`}>
        ...
        <button onClick={searchHandler} className="blueCol sm:text-sm text-xs px-4 h-10 whitespace-nowrap">Search</button>
        </div>
        </div>
        ...
        {/* Right section for company filter */}
        <div className={`flex justify-start flex-col p-4 rounded w-full sm:w-1/4 ${isMobile ? 'w-full' : 'w-1/4'}`}>
        <p className="text-xl flex justify-center underline underline-offset-4">Companies</p>
        <ul className="flex pt-3 justify-start md:justify-center items-center mb-3 flex-col gap-3">
        {companyData.map((e, i) => (
        <li
          key={i}
        ...
        <div className="flex flex-col gap-4 pt-5">
        <button onClick={() => rightFilter(jobs)} className="blueCol px-1 py-1 text-sm sm:text-base">Apply Filter</button>
        <button onClick={removeRightFilter} className="blueCol px-1 py-2 text-xs sm:text-sm">Remove Filter</button>
        </div>
        </div>
      </div>
      </>
